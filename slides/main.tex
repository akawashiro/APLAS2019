...
\begin{frame}[fragile]{What is Multi-stage Programming}
    ...
    \note{
        ...
        run-time simultaneously. Famous applications of multi-stage programming
        is speeding up programs using run-time information, which is also known
        as run-time specialization and DSLs, such as DSL for linear algebra or
        image processing.
    }
\end{frame}
...
\begin{frame}[fragile]{Example of Multi-stage Programming in MetaOCaml}
 ...
 - : int = 6
    ...
    \note{
        ...
        code value banded to "a". Tilde symbol is a symbol to embed a code
...
\begin{frame}[fragile]{Application of Multi-stage Programming: vadd-gen}
 ...
 (*   [v[0]+w[0];v[1]+w[1];v[2]+w[2]] >  *)
    ...
    \note{
        ...
        vadd-gen function takes a integer as the vector length and return a
        code value of function which takes two fixed length vectors and return
        the sum of them. For example, when I apply 3 to vadd-gen function and
        bind it to vadd3, vadd3 is a code value which takes two 3 length
        vectors. I show the contents of vadd3 in commented out lines. It is
        important that there is no loop. Loop is unrolled at generation of a
        code value and it helps to make an efficient function.
    }
\end{frame}
...
\begin{frame}[fragile]{The problem of vadd3 generated by vadd-gen}
 ...
 Exception: Failure
    ...
    \note{
        ...
        for vectors of wrong length. For example, when I apply correct length
...
\begin{frame}[fragile]{Our Approach: Dependent Types [Martin-L{\"o}f'73]}
 ...
 \end{Verbatim}
    ...
    \note{
        ...
        are types dependent on values and can prevent illegal value of vadd3.
        Using dependent types, we can express the length of vectors in their
        types. For example, a vector [1;2] can be typed as vec 2 and vadd3 can
        be typed code of function which takes values of vec 3 type and return a
        value of vec 3 type. Now, if I apply wrong length vectors, [1;2] and
        [4;5] to vadd3, I get only type error not run-time error.
    }
\end{frame}
...
\begin{frame}[fragile]{Our Contribution: \LMD}
    ...
    \end{itemize}
    \note{
        Our contribution of this research is designing lambda-M-D by
        introducing dependent types into multi-stage calculus lambda-T-P by
        Hanada and Igarashi. lambda-T-P have both of features of multi-stage
...
\begin{frame}{Contents}
    \tableofcontents[currentsection]
    \note{
        From the next slide, I explain existing multi-stage calculus lambda-T-P which we used as our lambda-M-D.
    }
\end{frame}
...
\begin{frame}[fragile]{Quick Overview of \LTP [Hanada\&Igarashi'14]}
    ...
    \note{
        ...
        value with a black triangle which corresponds to angle brackets in
        MetaOCaml. Left-directed black triangle is used to embed a code values
        which corresponds to tilde in MetaOCaml. Code type is represented with
        white triangle.
    }
\end{frame}
...
\begin{frame}[fragile]{Types of \LTP}
    ...
    \note{
        ...
        or function type. Typing judgements of lambda-TP have shape of M is
        typed tau at stage n under environment gamma.
    }
\end{frame}
...
\begin{frame}[fragile]{What is a stage?}
    ...
    \note{
        ...
        . For example, this whole term is on stage 0 because there is no black
        triangle around this term. However, the subterm lambda x dot x + 1 is
        on stage 1 because there is one black triangle around the subterm.
        Typing judgements and environments of lambda-TP include a stage.
    }
\end{frame}
...
\begin{frame}[fragile]{Typing Rules of \LTP}
    ...
    \note{
        ...
        restriction. It is the function and the argument must live in the same
        stage. Typing rules of quote and unquote is T-black triangle and T-left
        directed black triangle. They say that black triangles increase and
        decrease the stage of terms. In general, we cannot use terms of one
        stage at other stages.
    }
\end{frame}
...
\begin{frame}[fragile]{Cross Stage Persistence (CSP)}
    ...
    \note{
        But we want to use terms at different stage. Cross Stage Persistence
...
\begin{frame}[fragile]{Overview of \LMD}
    ...
    \note{
        ...
        calculus lambda-TP with dependent types. Its term are the same as
...
\begin{frame}[fragile]{Types and Kinds of \LMD}
    ...
    \note{
        ...
        type and application such as vec 3 type. There are kinds also in
...
\begin{frame}[fragile]{Kinding rules of \LMD}
    Kiding judgements also include stages.
    \begin{center}
        \( \G \vdash \tau :: K @ n \)
    \end{center}
    ...
    \note{
        ...
        such as vec n. It is natural that types dependent on a value live in
        the same stage of the value. So, application rule, K-App, says that if
        you want to apply term M to type sigma, they must live in the same
        stage n. K-Abs says when sigma is kinded at stage n and variable x in
        the environment is on stage n also, you can abstract x using Pi.
    }
\end{frame}
...
\begin{frame}[fragile]{CSP in Types}
    ...
    \end{center}
    So, we need CSP in types also in order to use \( n \) in \( \TW \).
    \begin{exampleblock}{}
        \begin{center}
            \( \lambda n:\text{int}. \text{zeros}(n): \underbrace{\Pi n:\text{int}. \TW (\overbrace{\text{vec}\ \%n}^{\text{stage }1})}_{\text{stage }0} \)
        \end{center}
    \end{exampleblock}
    \note{
        Code type decreases the stage as you can see in K-white triangle rule.
        So we need CSP in types also. For example, let's consider a function
        which takes integer n and returns a vector of length n filled with
        zero. Because n is a variable at stage 0, you must insert \% symbol
        when you use it in code type. But vec is used without \% symbol. I will
        explain the reason in the next slide.
    }
\end{frame}
...
\begin{frame}[fragile]{Technical challenge of vadd3}
    ...
    \note{
        But there is a technical challenge in vadd3. It is vadd3 is hard to
        ...
        values. However, double has type of code which takes vec 3. So we
...
\begin{frame}[fragile]{Solution}
    ...
    \note{
        ...
        equivalent type. So, vec \% 3 is equivalent to vec 3 and vadd3 can have
        type of code of function which takes two vec 3 values and returns a vec
        3 value. The type is equivalent to the original type of vadd3.
    }
\end{frame}
...
\begin{frame}[fragile]{Conclusion}
    ...
    \note{
        ...
        dependent types. And it dependent types is full dependent types not

...
\begin{center}
    ...
    \infrule{\G\V K\iskind @A}{\G\V K\iskind @A\alpha}{\WCsp}\\[2mm]
\end{center}

\figheader{Kinding}{\rulefbox{\G \V T::K}}
\begin{center}
    ...
    \infrule{\G\V \tau::K@A}{\G\V \tau::K@A\alpha}{\KCsp}
\end{center}

\figheader{Typing}{\rulefbox{\G\V t:\tau}}
\begin{center}
...
Basecases are \WStar, \KVar, \TVar, \QKRefl, \QTRefl, \QRefl.
We can prove for these cases directly from the definition of rules.

Others are proved by using induction hypothesis and a rule. We show the case of \KCsp as an example.
\begin{itemize}
    \item \KCsp
...
\begin{thm}[Generized Inversion Lemma for $\Pi$ type]
...
\begin{itemize}
    ...
    \item \TAbs
        ...
        Use "Agreement" to $\G, x:\sigma@A \V M:\tau@A$, get $\G, x:\sigma@A \V \tau :: * @A$.

        Then we can take $M, \sigma, \tau, *, *$ as $N, \sigma', \tau', K, J$ in the statement of the theorem.
...
\begin{thm}[Inversion Lemma for $\Pi$ type]
    If $\G \V (\lambda x:\sigma.N) : (\Pi x:\sigma'.\tau)@A$ then $\exists \tau$ such that
    \begin{enumerate}
        \item $\G \V \sigma \E \sigma'@A$
        \item $\G ,x:\sigma@A\V N:\tau@A$
        \item $\G, x:\sigma@A\V \tau \E \tau' @A$
    \end{enumerate}
\end{thm}

From the above theorem and \TConv.
...
\begin{thm}[Generized Inversion Lemma for $\TW$ type]
    This theorem is used to prove Inversion Lemma for $\Pi$ type.

    \begin{enumerate}
    \item If $\G \V M : \TW_\alpha \tau@A$ then $\exists N, \tau', K$ such that
    \begin{enumerate}
        \item $M = \TB_\alpha N$
        \item $\G \V N:\tau' @ A\alpha$
...
\begin{itemize}
    ...
    \item \TTB
        ...
        Use "Agreement" to $\G \V M : \tau@A\alpha$, get $\G \V \tau : *@A\alpha$.

        Then use \KCsp\ to $\G \V \tau : *@A\alpha$, get $\G \V \tau : *@A$.

        Then we can take $M, \tau, *$ as $N, \tau', K$ in the theorem statement correspondingly.

    \item Otherwise
    
        Just same as "Generized Inversion Lemma for $\Pi$ type".
...
\begin{thm}[Generized Inversion Lemma for $\forall$ type]
    This theorem is used to prove Inversion Lemma for $\forall$ type.

    \begin{enumerate}
    \item If $\G \V M : \forall\alpha.\tau@A$ then $\exists N, \tau', K$ such that
    \begin{enumerate}
        \item $M = \Lambda\alpha.N$
        \item $\G \V N:\tau' @ A$
        \item $\G \V \tau \E \tau' : K @A$
    \end{enumerate}
...
\begin{itemize}
    ...
    \item \TGen
        ...
        Use "Agreement" to $\G\V M:\tau@A$, get $\G\V \tau :: * @A$.

...
because \red{we develop a multistage calculus not a multitransition calculus.}

% Kidinding and Well-formed Kinding Rules

In order to extend \LMD with dependent types, we need kinding rules and well-formed kinding rules.
It was lucky that almost all rules are determined easily.
This is because multistage calculus and dependent types are almost \red{orthogonal}.
Therefore, we can get kinding rules and well-formed kinding rules of \LMD just by 
attaching stage anotations to ones of \LLF.
For example, \KAbs' is a kinding rule for a dependent type in \LLF and \KAbs\ is a corresponding one.
\begin{center}
	\infrule{\G\vdash \tau :: * \andalso \G,x:\tau@A\vdash \sigma::J}{\G\vdash(\Pi x:\tau.\sigma) :: (\Pi x:\tau.J)}{\KAbs'} \\[2mm]
	\infrule{\G\V \tau :: *@A \andalso \G,x:\tau@A\V \sigma::J@A}{\G\V(\Pi x:\tau.\sigma) :: (\Pi x:\tau.J)@A}{\KAbs} \\[2mm]
\end{center}

% Constants and Base Types

The way of handling of constants and base types is diffrence between \LMD and \LTP.
We adopt signature $\Sigma$ to handle constants and base types.
This is because signature simplify kinding rules relating to type variables.

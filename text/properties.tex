...
\AI{Did we define $\Delta [x \mapsto N]$?}
...
\begin{proof}
	...
	For each case, we can use straightforward induction on derivations. \AI{何の derivation?}
...
We prove this thorem by translating \LMD to the symply typed lambda calculus~\cite{Tsukada} \AI{what is a standard citation for a proof of SN of a dependently typed calculus?}.
...
Now, we turn our attention to proving Progress for staged semantics.
The following lemma Unique Decomposition ensures that, every typed
...
\begin{theorem}[Unique Decomposition]
  ...
  \begin{enumerate}
  \item $ M \in V^A$, or
  \item there uniquely exist $B, E^A_B$, and $R^B$ such that $M = E^A_B[R^B]$.
  \end{enumerate}
\end{theorem}

\begin{proof}
  We can prove by straightforward induction on typing derivations.
  Main cases are \TApp, \TTBL, and \TIns.
  We need Inversion Lemmas for them.
\end{proof}

In a usual setting, the type environment $\G$ has to be empty, namely
the term has to be closed.  The condition is relaxed here because
variables at stages higher than \(\varepsilon\) are considered
...
\AI{Don't we prove $\longrightarrow_s$ is a subrelation of $\longrightarrow$?}
...
The subscript $\alpha$ in $\TB_\alpha M$ is a \textit{stage
  variable}\footnote{%
  In Hanada and Igarash~\cite{Hanada2014}, it was called a
...
example, a signature for the Boolean type and constans is given as
follows $\B::*, \text{true}:\B, \text{false}:\B$ (where $*$ is the
kind of proper types).  Declarations in a siganature are not

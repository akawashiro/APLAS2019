...
Let's consider an application, for example, in computer graphics, in
which we have potentially many pairs of vectors of the fixed (but
statically unkown) length and a function---such as vector
addition---to be applied to all of them.

\AI{Some more description about what we'd like to do.}
\AI{Brief comparison with other ways (somewhere else)?}
...
The function vadd (with an auxiliary
function vadd$_1$) below implements a code generator.
...
There are other ways to realize the vector addition function for fixed-length
lists: determining statically the length of all the vectors and use tuples
instead of lists or checking dynamically the dimensions of every pair of
lists. However, our method is better than there two methods in two points.
First, our function, $\text{vadd}_1$ can generate function for arbitrary
length vectors unlike ones using tuples. Second, $\text{vadd}_1$ has an
advantage in speed over ones using dynamic checking because it can generate
optimized function for a given length.

\paragraph{Remark:}
When this function is composed with another piece of code $v$ of type, say,
\(\TW_\gamma \text{Vector }5\), \QPercent{} plays an essential role; that is,
\(\text{Vector }5\) and \(\text{Vector }(\%_\beta 5)\) are syntactically
different types but \QPercent{} enables to equate one with another.

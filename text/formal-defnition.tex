...
%     \textrm{Constants}       &   &                          & c                                                                                                          \\
    \textrm{Terms}           &   & M,N,L,O,P                & ::= c \mid x \mid \lambda x:\tau.M\ \mid M\ M \mid \TB_\alpha M                                            \\
                             &   &                          & \ \ \ \ \mid \TBL_\alpha M \mid \Lambda\alpha.M \mid M\ \varepsilon \mid \%_\alpha M                          \\  
...
$M\ \varepsilon$ is an application of $\varepsilon$ to a stage variable \AI{I think the usage of ``application'' is wrong again}; and 
...
We adopt a \LLF-like system, where constants and type-level constants for base types \AI{``for base types''?} are given in the signature $\Sigma$, which 
...
\begin{align*}
	...
	% (\Lambda\beta.M)[\alpha \mapsto \varepsilon]   & = \Lambda\beta.M                                                     & (\text{if } \alpha = \beta)    \\
...
Application of a non-$\varepsilon$ stage to a stage abstraction is prohibited in order to simplify \LMD. \AI{This should have been mentioned earlier in the comparison with Hanada--Igarashi.}
...
\begin{figure}
	\begin{center}
		...
		\infrule[\TIns]{\G\V M:\forall\alpha.\tau @A}{\G\V M\ \varepsilon:\tau[\alpha \mapsto \varepsilon]@A} \andalso

...
function with respect to the size of vectors.  The following function
vadd (with an auxilirary function vadd$_1$) implements a code
...
We assume that
$\text{cons}$ is of type $\I \to (\text{Vector}\ n) \to (\text{Vector}\ (n+1))$, 
$\text{head}$ is of type $\text{Vector}\ n \to \I$, and
$\text{tail}$ is of type $(\text{Vector}\ n) \to (\text{Vector}\ (n-1))$ for any integer $n$.

% type checker can know $n=0$ when process the $\textbf{then}$ clause,

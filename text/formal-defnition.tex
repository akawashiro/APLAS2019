...
\red{$\text{head}: \text{Vector}\ n \to \I$としているが
$\text{head}: \Pi n:\I.\text{Vector}\ n \to \I$のほうが良いか? tail, consも同様}
...
which takes a length of integer vectors and returns addition function for vectors with the length.
...
$\text{head}$ is of type $\text{Vector}\ n \to \I$,
$\text{tail}$ is of type $(\text{Vector}\ n) \to (\text{Vector}\ (n-1))$, 
and  $\text{cons}$ is of type $\I \to (\text{Vector}\ n) \to (\text{Vector}\ (n+1))$, 
...
$\text{vadd}$ function has the type of $\Pi n:\I.\F\beta.\TW_\beta(\Vpn\to\Vpn\to\Vpn)$ and
...
For example, if we want to composite $\text{vadd5}$ with code of reverse fucntion,
...
and compose the code values of functions as we expected.

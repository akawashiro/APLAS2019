...
\begin{lemma}[Weakening]
  \begin{enumerate}\ 
  \item	If \(\G \V M:\tau@A\), then \(\G, z:\xi@B \V M:\tau@A\). 
\AI{Apply similar changes and remove \%.}
\item If \(\G \V M:\tau@A\), then \(\G, Z:G@B \V M:\tau@A\).  \AI{Type variables do not appear in type environments any longer.  This one is obsolete.}

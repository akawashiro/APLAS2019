...
Multi-stage programming enables us to generate and run fragments of code at runtime.  \AI{It sounds like the subject of ``generate and run'' is human being (``us'').  Something like ``Multi-stage programming offers linguistic support for dynamic code generation and execution.''?}
MetaOCaml~\cite{oleg2014} \AI{You need a space between a word and a citation.  Taha's original MetaOCaml should be cited, too.} provides features for multi-stage programming, which are brackets, escape, and run.  \AI{CSP?}
...
\AI{Why SEGV?  MetaOCaml will raise an exception.}
...
\AI{このイントロ、各段落はまあまあよいのですが、全体の流れが、ふたつ目の文から MetaOCaml という特定の多段階プログラミング言語の話をして、それから応用と問題点の話になりますが、ここが多段階プログラミング一般の話なのか MetaOCaml 固有の話なのかがぶれているように思います。まずは、多段階プログラミング一般の話を応用も含めてする。(他の多段階計算ができるシステムを無視しない。)それから、MetaOCaml の紹介と、その型システムの問題点の話をすべきでしょう。MetaOCaml の型システムでできることもふれておいた方がよいでしょう。}

\subsection{Dependent Types}

% \subsubsection{依存型とは何か?}

Dependent types are types which is \AI{are} dependent on values.
We can use dependent types for securer programming. \AI{Cite Xi--Pfenning, for example?}
...
\AI{くっつけましょう。Section 1 で指摘した問題を依存型で解決するというのは自然なアイデアだし。One natural idea to address this problem is the introduction of dependent types. みたいなこといって、依存型の導入をする。先行研究として Brady--Hammond にもふれる。}
% \subsubsection{「生成した行列計算のコードは特定のサイズに特化しており、他のサイズでは使えない。
...
\LMD makes multi-stage programming more safer \AI{remove ``more''} with the power of dependent types.

\AI{Give a summary of our technical contribution as itemization.}
...
Section 4 shows the properties of \LMD including Unique Decomposition \AI{There are more important properties that should be mentioned!  In fact, the properties should be mentioned earlier.}.
...
\AI{Related work is discussed in Section 5?  Section 6 is the section for conclusion, which includes future work.}
